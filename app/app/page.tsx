...
export default function DashboardPage() {
  ...
  const handleEnterPrototype = () => { setShowSplash(false) }
  const handleShowSplash = () => { setShowSplash(true) }

  if (showSplash) return <SplashScreen onEnter={handleEnterPrototype} />
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-[#272C41]" : "bg-[#F6F8FA]"}`}>
      <Header onShowSplash={handleShowSplash} showingDashboard={!showSplash} dark={isDark} />
